...
import { Aquifer, WellFunction, ExchangeSDK } from '../src';
...
describe('Add Liquidity', () => {
  beforeEach(async () => {
    testWell = await setupWell([ExchangeSDKInstance.tokens.PINTO, ExchangeSDKInstance.tokens.USDC], account);
  ...
  describe('addLiquidity', () => {
    it('should obtain quote add liquidity to the Well', async () => {
      const tokenAmountsIn: TokenValue[] = [
        ExchangeSDKInstance.tokens.PINTO.amount(100),
        ExchangeSDKInstance.tokens.USDC.amount(100)
      ];
