...
Forward means selling, for ex, I want to spend 1 WETH, give me as many PINTOS as possible.
Reverse means buying, for ex, I want to receive exactly 1000 PINTOS, tell me the amount of WETH I need to spend.

Single step means we are swapping against only one well, ie, the in and out tokens belong to the same well. For ex, swapping WETH <> PINTO in a WETH_PINTO well

Multistep means we need to route through multiple wells, for ex, USDC > PINTO, going through the USDC_WETH well then through the WETH_PINTO well; two swaps.
...
WETH > PINTO > USDC > DAI, so a reverse quote looks like this:
...
- PINTO ? > 100.106006 DAI = 100.001202 USDC
- Apply slippage to usdc, so 100.001202 \* 1.001 = 100.1012032 PINTO
- WETH ? > 100.1012032 PINTO might quote 0.050000903012060272 PINTO.
