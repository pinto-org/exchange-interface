...
import { ExchangeSDK } from 'src/index';
...
export class UnWrapEthStep implements SwapStep {
  ...
  constructor(sdk: ExchangeSDK, weth9: WETH9, fromToken: Token, toToken: Token) {
    this.sdk = sdk;
  ...
  swapMany(recipient: string, minAmountOut: TokenValue): Operation {
    ...
    if (this.direction !== Direction.FORWARD)
      throw new Error('swapMany() can only be called for quotes where direction was Forward');
