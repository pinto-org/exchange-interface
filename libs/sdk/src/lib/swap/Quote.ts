...
export class Quote {
  ...
  constructor(sdk: ExchangeSDK, fromToken: Token, toToken: Token, route: Route, account: string) {
  ...
  private async prepareSingle(recipient: string, deadline: number) {
    ...
    const { contract, method, parameters } = step.swapSingle(
    ...
    const doSwap = (overrides?: TxOverrides): Promise<ContractTransaction> =>
      // @ts-ignore
      contract[method](...getParameters(overrides));
  ...
  private async prepareMulti(recipient: string): Promise<QuotePrepareResult> {
    ...
    if (this.direction !== direction)
      throw new Error('Direction of last quote does not match expected direction of swap');
  ...
  private async prepareMultiReverse(recipient: string, deadline: number): Promise<QuotePrepareResult> {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      const { contract, method, parameters } = step.swapManyReverse(
        nextRecipient,
        maxAmountOut,
