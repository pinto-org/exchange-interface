...
describe('Remove Liquidity', () => {
  beforeEach(async () => {
    testWell = await setupWell([wellsSdk.tokens.PINTO, wellsSdk.tokens.WETH], account);
  ...
  describe('removeLiquidity', function () {
    it('should remove liquidity in a balanced ratio', async function () {
      ...
      const initialPINTOBalance = await wellsSdk.tokens.PINTO.getBalance(account);
      ...
      const actualPINTOBalanceAfterRemove = await wellsSdk.tokens.PINTO.getBalance(account);
  ...
  describe('removeLiquidityImbalanced', function () {
    it('should remove liquidity in an imbalanced ratio', async function () {
      ...
      // Get the expended token amounts
      const PINTOAmount: TokenValue = wellsSdk.tokens.PINTO.amount(1000);
