...
import { TokenValue, ERC20Token } from '@exchange/sdk-core';
...
export class SwapBuilder {
    ...
    for (const [i, node] of this.#nodes.entries()) {
      ...
      if (isERC20Node(node)) {
        if (isWellNode(node)) {
          ...
          this.#advPipe.add(node.buildStep({ copySlot: this.#getPrevNodeCopySlot(i) }), { tag: node.tag });
          ...
          this.#advPipe.add(node.buildStep({ recipient: SwapBuilder.sdk.contracts.pipeline.address }), {
            tag: node.tag
          });
        } else {
  ...
  #loadPipeline(node: SwapNode, caller: string, from: FarmFromMode) {
    const callerIsPipeline = caller.toLowerCase() === SwapBuilder.sdk.contracts.pipeline.address.toLowerCase();
    if (!callerIsPipeline) {
      const loadToken = isWrapEthNode(node) ? node.buyToken : (node as ERC20SwapNode).sellToken;

      const transfer = new SwapBuilder.sdk.farm.actions.TransferToken(
        loadToken.address,
        SwapBuilder.sdk.contracts.pipeline.address,
        from,
        FarmToMode.EXTERNAL
      );
  ...
  #offloadPipeline(node: SwapNode, recipient: string, fromMode: FarmFromMode, toMode: FarmToMode, i: number) {
    const recipientIsPipeline = recipient.toLowerCase() === SwapBuilder.sdk.contracts.pipeline.address.toLowerCase();
    ...
    const approve = new SwapBuilder.sdk.farm.actions.ApproveERC20(outToken, SwapBuilder.sdk.contracts.diamond.address, {
      ...prevActionClipboard,
      pasteSlot: 1
    });

    const transfer = new SwapBuilder.sdk.farm.actions.TransferToken(outToken.address, recipient, fromMode, toMode, {
      ...prevActionClipboard,
      pasteSlot: 2
    });
  ...
  #getApproveERC20MaxAllowance(node: SwapNode) {
    if (isUnwrapEthNode(node)) {
      const approve = new SwapBuilder.sdk.farm.actions.ApproveERC20(node.sellToken, node.allowanceTarget);
  ...
  #initWorkFlows(advFarmName?: string, advPipeName?: string) {
    this.#advFarm = SwapBuilder.sdk.farm.createAdvancedFarm(advFarmName);
    this.#advPipe = SwapBuilder.sdk.farm.createAdvancedPipe(advPipeName);
    return this;
  }
}
