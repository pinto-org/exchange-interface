...
import { BeanstalkPrice } from 'src/constants/generated/Ecosystem/DiamondPrice';
...
export class SwapPriceCache {
  ...
  hasReservesAndPrices(well: BasinWell) {
    ...
    const nonBeanUnderlying = well.getPairToken(SwapPriceCache.sdk.tokens.PINTO);
  ...
  private async refreshPricesAndPoolDatas() {
    const response = await SwapPriceCache.sdk.contracts.diamond.callStatic.advancedPipe(
      this.constructRefreshPipeCalls(),
      "0"
    );
    SwapPriceCache.sdk.debug("[BeanSwapV2/refreshReservesAndPrices] AdvPipeResult:", response);
  ...
  private constructRefreshPipeCalls() {
    ...
    // Beanstalk Price
    pipeCalls.push({
      target: SwapPriceCache.sdk.contracts.price.address,
      callData: SwapPriceCache.sdk.contracts.price.interface.encodeFunctionData("price"),
    ...
    this.getWellsNonBeanUnderlying().forEach((token) => {
      pipeCalls.push({
        target: SwapPriceCache.sdk.contracts.diamond.address,
        callData: SwapPriceCache.sdk.contracts.diamond.interface.encodeFunctionData("getTokenUsdPrice", [token.address]),
        clipboard: Clipboard.encode([])
      });
    });

    SwapPriceCache.sdk.debug("[SwapPriceCache/constructRefreshPipeCalls] PipeCalls: ", pipeCalls);
  ...
  private processRefreshPipeCallResults(results: string[]) {
    ...
    tokenPrices.set(SwapPriceCache.sdk.tokens.PINTO, priceResult.beanPrice);
    ...
    for (const [i, token] of this.getWellsNonBeanUnderlying().entries()) {
      ...
      if (token.equals(SwapPriceCache.sdk.tokens.WETH)) {
        tokenPrices.set(SwapPriceCache.sdk.tokens.ETH, decodedPrice);
      }
    }
  ...
  private decodeGetTokenUsdPrice(result: string) {
    try {
      const priceDecoded = SwapPriceCache.sdk.contracts.diamond.interface.decodeFunctionResult(
        "getTokenUsdPrice", 
        result
      )[0];
      ...
      SwapPriceCache.sdk.debug(`[SwapPriceCache/decodeGetTokenUsdPrice] Error decoding getTokenUsdPrice`, e);
      throw e;
    }
  }
  ...
  private decodeBeanstalkPrice(result: string) {
    ...
    try {
      const decoded = SwapPriceCache.sdk.contracts.price.interface.decodeFunctionResult(
        "price", 
        result
      )[0] as BeanstalkPrice.PricesStructOutput;

      const beanPrice = SwapPriceCache.sdk.tokens.PINTO.fromBlockchain(decoded.price);

      for (const pool of decoded.ps) {
        const well = SwapPriceCache.sdk.pools.getWellByLPToken(pool.pool);
        if (!well) {
          continue;
        }
        ...
        const data: PricePoolData = {
          ...
          lpBdv: SwapPriceCache.sdk.tokens.PINTO.fromBlockchain(pool.lpBdv)
        };

        poolData.set(well, data);
      }

      SwapPriceCache.sdk.debug("[SwapPriceCache/decodeBeanstalkPrice]: RESULT", {
        beanPrice,
        poolData
      });
      ...
      SwapPriceCache.sdk.debug("Error decoding Beanstalk Price contract result", e);
      throw e;
    }
  }

  private getWellsNonBeanUnderlying() {
    const wells = SwapPriceCache.sdk.pools.getWells();

    const tokens = new Set<ERC20Token>(wells.map((well) => well.tokens).flat());

    tokens.delete(SwapPriceCache.sdk.tokens.PINTO);
    return [...tokens];
  }
}
