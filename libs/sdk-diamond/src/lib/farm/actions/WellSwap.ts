...
export class WellSwap extends StepClass<AdvancedPipePreparedResult> {
  ...
  async run(_amountInStep: ethers.BigNumber, context: RunContext): Promise<Step<AdvancedPipePreparedResult>> {
    const well = await WellSwap.sdk.pools.getWellByLPToken(this.wellAddress);

    if (!well) throw new Error('swap: well not found');
    ...
    return {
      ...
      prepare: () => {
        ...
        if (!reversed) {
          ...
          callData = well
            .getContract()
            .interface.encodeFunctionData('swapFrom', [
          ...
          callData = well
            .getContract()
            .interface.encodeFunctionData('swapTo', [
              this.fromToken.address,
              this.toToken.address,
      ...
      decode: (data: string) => well.getContract().interface.decodeFunctionData(wellFunctionName, data),
      // @ts-ignore
