...
> {
  ...
  static slip(_amount: ethers.BigNumber, _slippage: number) {
    ...
    return _amount
      .mul(Math.floor(Workflow.SLIPPAGE_PRECISION * (1 - _slippage / 100)))
      .div(Workflow.SLIPPAGE_PRECISION);
  }
  ...
  protected _copy<T extends Workflow<EncodedResult>>(
    WorkflowConstructor: new (...args: ConstructorParameters<typeof Workflow>) => T
  ) {
  ...
  protected async buildStep(
    ...
    try {
      if (input instanceof Workflow) {
        ...
        const fnResult = (await input.call(this, amountInStep, context)) as
          | Step<PreparedResult>
          | PreparedResult
          | string;
    ...
    const run = async (i: number, label: 'estimate' | 'estimateReversed') => {
      ...
      const skip = options?.skip
        ? typeof options.skip === 'function'
          ? await options.skip(nextAmount, context)
          : options.skip
        : false;
