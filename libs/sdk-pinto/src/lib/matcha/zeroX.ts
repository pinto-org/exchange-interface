...
export class ZeroX {
  ...
  readonly swapV1Endpoint = 'https://base.api.0x.org/swap/v1/quote';
  ...
  async quote<T extends ZeroExAPIRequestParams = ZeroExAPIRequestParams>(
    ...
    const requests = fetchArgs.map((params) => {
      const urlParams = new URLSearchParams(this.generateQuoteParams(params) as unknown as Record<string, string>);
  ...
  private async send0xRequest(urlParams: URLSearchParams, requestInit?: Omit<RequestInit, 'headers' | 'method'>) {
    const options = {
      ...requestInit,
      method: 'GET',
      headers: new Headers({
