...
export class TransferToken extends StepClass<BasicPreparedResult> {
  ...
  async run(_amountInStep: ethers.BigNumber, context: RunContext) {
    return {
      ...
      decode: (data: string) =>
        TransferToken.sdk.contracts.beanstalk.interface.decodeFunctionData('transferToken', data),
      decodeResult: (result: string) =>
        TransferToken.sdk.contracts.beanstalk.interface.decodeFunctionResult('transferToken', result)
    };
  }
}
