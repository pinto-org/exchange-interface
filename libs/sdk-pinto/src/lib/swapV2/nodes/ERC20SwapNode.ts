...
import { PintoSDK } from 'src/lib/PintoSDK';
...
export abstract class ERC20SwapNode extends SwapNode implements IERC20SwapNode {
  ...
  constructor(sdk: PintoSDK, sellToken: ERC20Token, buyToken: ERC20Token) {
    super(sdk);
...
export class WellSwapNode extends ERC20SwapNode {
  ...
  constructor(sdk: PintoSDK, well: BasinWell, sellToken: ERC20Token, buyToken: ERC20Token) {
...
export class ZeroXSwapNode extends ERC20SwapNode {
  ...
  async quoteForward(sellAmount: TokenValue, slippage: number) {
    this.setFields({ sellAmount, slippage });
    this.validateQuoteForward();
    this.validateTokenIsNotPINTO(this.sellToken);
    this.validateTokenIsNotPINTO(this.buyToken);
  ...
  private validateTokenIsNotPINTO(token: ERC20Token) {
    if (token.equals(ZeroXSwapNode.sdk.tokens.PINTO)) {
      throw this.makeErrorWithContext('Cannot swap PINTO tokens via 0x. For PINTO quotes, use WELLS instead.');
    }
  }
}
...
export class WellSyncSwapNode extends ERC20SwapNode {
  ...
  declare readonly sellToken: ERC20Token;

  declare readonly buyToken: ERC20Token;
  ...
  constructor(sdk: PintoSDK, well: BasinWell, sellToken: ERC20Token, buyToken: ERC20Token = well.lpToken) {
    super(sdk, sellToken, buyToken);
    this.well = well;
    this.allowanceTarget = this.well.address;
  }
  ...
  buildStep({ recipient }: WellSyncSwapBuildParams): StepClass<AdvancedPipePreparedResult> {
    this.validateAll();
    const sync = new WellSyncSwapNode.sdk.farm.actions.WellSync(this.well, this.sellToken, recipient);
