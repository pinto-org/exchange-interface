...
function PumpDetails({ well }: Props) {
	...
	return (
		<TextContainer>
			<div>
				Pumps are the oracle framework of the Pinto Exchange. Well deployers can
				define the conditions under which the Well should write new reserve data
				to the Pump, which can be used as a data feed.
			</div>
			{isMultiFlow && (
				<div>
					The{" "}
					...
					is attached to{" "}
					{well?.tokens
						? `the ${formatWellTokenSymbols(well)} Well`
						: "this well"}
					.
				</div>
			)}
